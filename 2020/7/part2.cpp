...
class Node {
	...
	public:
		...
		// Count the number of individual bags this node (bag) contains
		size_t countContainedBags() {
			// Recursively count connected nodes
			size_t count = 0;
			for (Edge & e : this->edges) {
				size_t a = e.weight*(e.node->countContainedBags() + 1);
				count += a;
			}
...
int main(void) {
	...
	if (wanted == nullptr) {
		...
		size_t count = wanted->countContainedBags();
		std::cout << "One '" << wantedBag << "' bag must contain " << count << " other bags" << std::endl;
	}
